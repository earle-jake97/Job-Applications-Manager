...
export default function ApplicationItem({ application, onStatusChange, onDelete, onEdit, disabled, isDeleting, isUpdating }: ApplicationItemProps) {
  ...
  return (
    <tr className="border-b border-slate-200/80 transition-colors last:border-0 even:bg-slate-50/60 hover:bg-blue-50/40">
      <th scope="row" className={cell + 'max-w-56 text-left font-semibold break-words text-slate-900'}>{application.company}</th>
      <td className={cell + 'max-w-64 break-words text-slate-700'}>{application.position}</td>
      ...
      <td className={cell}>
        ...
        {isUpdating && <span role="status" className="mt-1 block text-xs text-slate-500">Saving…</span>}
      </td>
      ...
      <td className={cell + 'w-56 max-w-64 text-sm text-slate-600'}>
        {application.notes ? <details>
          <summary className="cursor-pointer break-words leading-6 marker:text-slate-400">{application.notes.length > 55 ? application.notes.slice(0, 55) + '…' : application.notes}</summary>
          <p className="mt-2 whitespace-pre-wrap break-words rounded-md bg-slate-100 p-3 leading-6">{application.notes}</p>
        </details> : <span className="text-slate-400" aria-label="No notes">—</span>}
      </td>
      <td className={cell + 'w-44 text-sm'}>
        {confirmDelete ? <div className="min-w-36 rounded-lg border border-red-200 bg-red-50 p-2">
          <p className="mb-2 font-medium text-red-900">Delete this entry?</p>
          <div className="flex gap-2">
            <button type="button" disabled={disabled} onClick={async () => { await onDelete(application.id); setConfirmDelete(false) }} className="min-h-9 rounded-md bg-red-700 px-2 font-medium text-white hover:bg-red-800 disabled:opacity-50">{isDeleting ? 'Deleting…' : 'Delete'}</button>
            <button type="button" disabled={disabled} onClick={() => setConfirmDelete(false)} className="min-h-9 rounded-md px-2 text-slate-700 hover:bg-white disabled:opacity-50">Cancel</button>
          </div>
